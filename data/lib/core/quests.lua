			...
			missions = {
				[1] = {
					name = "Power", storageid = Storage.FirstDragon.DragonCounter, startvalue = 0, endvalue = 200, description = function(player)
						return string.format("You already hunted %d/200 dragons.", (math.max(player:getStorageValue(Storage.FirstDragon.DragonCounter), 0)))
					end,
				},
		...
		[40] = {
			...
			missions = {
				[1] = {
                    ...
                    return string.format("James asked you to enter the cave for hunting 50 empowered minotaurs. Then he will be able to continue his research.\nMinotaurs killed: %d/50", (math.max(player:getStorageValue(Storage.CultsOfTibia.Minotaurs.jamesfrancisTask), 0)))
                end,
				},
				...
				[4] = {
					...
					states = {
						...
						[3] = function(player)return string.format("You survived the Trial. Barkless now have the right to see the cult leader but a powerful relic is sealing the path.\nBarkless markings broken to reverse the power of the cult object: %d of 10", (math.max(player:getStorageValue(Storage.CultsOfTibia.Barkless.Objects), 0)))
                end,
				...
				[5] = {
					...
					states = {
						...
						[3] = function(player) return string.format("While rubbish, the amulet you equipe emits a strange aura of splendour. You feel an urge to fulfill the amulets hunger for especific deaths... \nExorcisms: %d/5 ", (math.max(player:getStorageValue(Storage.CultsOfTibia.Misguided.Exorcisms), 0)))
						end,
				...
				[8] = {
					...
					states = {
						[1] = function(player)
						return string.format("A remnant cult of Zathroth has formed in a forbidden temple beneath Carlin. Find out what's behind this and stop it in time.\nTemporarily vaporised souls:\n%d Decaying\n%d Withering", (math.max(player:getStorageValue(18551), 0)), (math.max(player:getStorageValue(18550), 0)))
						end,
						[2] = "ASD",
			...
			missions = {
				[1] = {
					name = "The Great Dragon Hunt", storageid = Storage.AdventurersGuild.GreatDragonHunt.WarriorSkeleton, startvalue = 0, endvalue = 2, description = function(player)
						return string.format("You are exploring the Kha'zeel Dragon Lairs. Others obviously found a terrible end here. But the dragon hoards might justify the risks. You killed %d/50 dragons and dragon lords.", (math.max(player:getStorageValue(Storage.AdventurersGuild.GreatDragonHunt.DragonCounter), 0)))
					end,
				},
			},
		},
